...
pub fn normalize_inherent_projection<'a, 'b, 'tcx>(
    ...
    for (predicate, span) in predicates {
        let predicate = normalize_with_depth_to(
            ...
            cause.clone(),
            depth + 1,
            ty::Unnormalized::new_wip(predicate.skip_norm_wip()),
            obligations,
        );
                        ...
                        let tail = selcx.tcx().struct_tail_raw(
                            ...
                            |ty| {
                                ...
                                normalize_with_depth(
                                    ...
                                    ty::Unnormalized::new_wip(ty.skip_norm_wip()),
                                )
                                .value
                            },
                            || {},
...
fn confirm_coroutine_candidate<'cx, 'tcx>(
    ...
    let Normalized { value: coroutine_sig, obligations } = normalize_with_depth(
        ...
        obligation.cause.clone(),
        obligation.recursion_depth + 1,
        ty::Unnormalized::new_wip(coroutine_sig),
    );
...
fn confirm_iterator_candidate<'cx, 'tcx>(
    ...
    let Normalized { value: gen_sig, obligations } = normalize_with_depth(
        ...
        obligation.cause.clone(),
        obligation.recursion_depth + 1,
        ty::Unnormalized::new_wip(gen_sig),
    );
...
fn confirm_closure_candidate<'cx, 'tcx>(
    ...
    let Normalized { value: closure_sig, obligations } = normalize_with_depth(
        ...
        obligation.cause.clone(),
        obligation.recursion_depth + 1,
        ty::Unnormalized::new_wip(closure_sig),
    );
