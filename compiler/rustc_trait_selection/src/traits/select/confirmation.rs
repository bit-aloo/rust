...
impl<'cx, 'tcx> SelectionContext<'cx, 'tcx> {
    ...
    fn confirm_object_candidate(
        ...
        let upcast_trait_ref = normalize_with_depth_to(
            ...
            obligation.cause.clone(),
            obligation.recursion_depth + 1,
            Unnormalized::new_wip(upcast_trait_ref),
            &mut nested,
        );
            ...
            .map(Unnormalized::skip_norm_wip)
        {
            let normalized_supertrait = normalize_with_depth_to(
                ...
                obligation.cause.clone(),
                obligation.recursion_depth + 1,
                Unnormalized::new_wip(supertrait),
                &mut nested,
            );
        ...
        Ok(match (source.kind(), target.kind()) {
            ...
            (&ty::Adt(def, args_a), &ty::Adt(_, args_b)) => {
                ...
                let source_tail = normalize_with_depth_to(
                    ...
                    obligation.cause.clone(),
                    obligation.recursion_depth + 1,
                    Unnormalized::new_wip(tail_field_ty.instantiate(tcx, args_a).skip_norm_wip()),
                    &mut nested,
                );
                let target_tail = normalize_with_depth_to(
                    ...
                    obligation.cause.clone(),
                    obligation.recursion_depth + 1,
                    Unnormalized::new_wip(tail_field_ty.instantiate(tcx, args_b).skip_norm_wip()),
                    &mut nested,
                );
